...
var _ = Describe("Worker Reconcile", func() {
	...
	DescribeTable("Reconcile function", func(t *test) {
		reconcilr := worker.NewReconciler(nil, t.fields.actuator)
		expectInject(inject.ClientInto(t.fields.client, reconcilr))
		//reconcilr.(inject.Client).InjectClient(t.fields.client)
		expectInject(inject.InjectorInto(inject.Func(func(i interface{}) error {
			...
			// r.(inject.Client).InjectClient(t.fields.client)
			// r.(inject.Stoppable).InjectStopChannel()
			return nil
		}), reconcilr))

		got, err := reconcilr.Reconcile(t.args.request)
