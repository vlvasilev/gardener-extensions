...
func (m *machineToObjectMapper) Map(obj handler.MapObject) []reconcile.Request {
	ctx := context.TODO()
	fmt.Println("IN MACHINE-TO-OBJECT MAPPER")
	if obj.Object == nil {
		return nil
	}
	...
	if !ok {
		fmt.Println("CAN'T CAST CAST OBJECT TO *machinev1alpha1.Machine")
		return nil
	}
	...
	fmt.Printf("MACHINE-TO-OBJECT Mapper\n")
	return getReconcileRequestsFromObjectList(objList, m.predicates)
}
...
func getReconcileRequestsFromObjectList(objList runtime.Object, predicates []predicate.Predicate) []reconcile.Request {
	...
	utilruntime.HandleError(meta.EachListItem(objList, func(obj runtime.Object) error {
		...
		fmt.Printf("MAKE REQUEST FOR OBJECT \"%s\" NAMESPACE \"%s\" NAME '%s'\n", obj.GetObjectKind().GroupVersionKind().Kind, accessor.GetNamespace(), accessor.GetName())
